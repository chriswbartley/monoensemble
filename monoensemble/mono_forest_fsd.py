...
from monoensemble.tree import DecisionTreeClassifier
import numpy as np


class MonoRandomForestClassifierFSD(ForestClassifier):
    ...
    def __init__(self,
        ...
        if not self.mt_feat_types is None:
            self.mt_feat_types=np.asarray(self.mt_feat_types)
            if np.all(self.mt_feat_types==0):
                self.mt_feat_types=None
#        self.incr_feats = incr_feats
#        self.decr_feats = decr_feats
#        self.n_feats=n_feats
#        self.mt_feat_types = np.zeros(n_feats,dtype=np.int32)
