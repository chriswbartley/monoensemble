...
from sklearn.base import ClassifierMixin  # ..base import ClassifierMixin
from sklearn.base import RegressorMixin  # ..base import RegressorMixin
from sklearn.externals import six  # ..externals import six

from _mono_gradient_boosting import predict_stages
from _mono_gradient_boosting import predict_stage
...
# For Constr Log Regn: SCIPY NEWTON CG SOLVER
import scipy as sp
...
class LogOddsEstimator(object):
    ...
    def predict(self, X):
        ...
        if np.isscalar(self.prior):
            ...
            #y = np.empty((X.shape[0], self.n_classes-1), dtype=np.float64)
...
class LossFunction(six.with_metaclass(ABCMeta, object)):
    ...
    def __init__(
            ...
            incr_feats=[],
            decr_feats=[],
            coef_calc_type='logistic'):
        self.K = n_classes
        self.incr_feats = np.asarray(incr_feats)
        self.decr_feats = np.asarray(decr_feats)
        self.coef_calc_type = 0 if coef_calc_type == 'boost' else 1 if coef_calc_type == 'chkd_boost' else 3 if coef_calc_type == 'logistic' else 2  # bayes
...
#
    ...
    ## NO LONGER USED, USES CYTHON VERSIONS FOR SPEED ##
            ...
            if self.coef_calc_type == 0:
                ...
                if np.sign(numerator) * np.sign(denominator) * \
                    ...
                    if abs(denominator) < 1e-150:
                        ...
                        coef_ = numerator / denominator
        else:  # if self.coef_calc_type=='bayes':
            lidstone_alpha = 0.01
    ...
    def update_terminal_rules(
        ...
        """
        # compute leaf for each sample in ``X``
#        X_masked=X[sample_mask,:]
#        y_masked=y[sample_mask]
#        residual_masked=residual[sample_mask]
        rule_mask = apply_rules(
            ...
            X_leaf_node_ids,
            node_rule_map)
        if leaf_lower_corners is None:
            leaf_mask = None
        else:
            leaf_mask = apply_rules(
                X,
                leaf_lower_corners,
                leaf_upper_corners,
                X_leaf_node_ids,
        ...
        if self.coef_calc_type == 3:  # 'logistic':
            ...
            # residual_=residual[sample_mask]
            ...
            # np.hstack([rule_mask_,pred_.reshape([-1,1])])
...
def extract_rules_from_tree(tree, num_feats, incr_feats, decr_feats):
    ...
    if len(decr_feats) > 0:
        for i_mt in decr_feats - 1:
            rule_lower_corners[leaf_values > 0, i_mt] = RULE_LOWER_CONST
            # filter global rules (after monotonising)
            rule_upper_corners[leaf_values < 0, i_mt] = RULE_UPPER_CONST
        ...
        for k in range(k_max):
            ...
            if X_csr is not None:
                ...
                X_leaf_node_ids = tree.apply(
                    X, check_input=False).astype(
                    np.int32)
                if self.coef_calc_type == 'chkd_boost':
                    intercept_ = loss.update_terminal_rules(leaf_lower_corners,
                                                            leaf_upper_corners,
                                                            rule_lower_corners,
                                                            rule_upper_corners,
                                                            leaf_values,
                    ...
                    intercept_ = loss.update_terminal_rules(None,
                                                            None,
                                                            rule_lower_corners,
                                                            rule_upper_corners,
                                                            leaf_values,
...
#            if self.coef_calc_type=='logistic' and self.n_estimators==1:
#                self.init_=ZeroEstimator()
            # self.init_.fit(X, np.asarray([0,1]), np.asarray([1,1])) # special case with p=0.5 prior, because intercept is calculated in logistic regression
            # else:
